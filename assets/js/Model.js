...
export const updateEntireFilters = (
  filtersObj = { categories: [], date: Date.now() }
) => {
...
export const paginationTransactions = (page, transactions) => {
  state.pagination.maxPages = Math.ceil(
    transactions.length / state.pagination.resultsPerPage
  );
...
export const budgetPieChartCalculation = () => {
  ...
  const totalExpense = expenseArr.reduce((acc, a) => {
    return (acc += a.value);
  }, 0);
...
export const initFilter = (categories = []) => {
  ...
  if (state.currentAccount.movements.length > 0) {
    filters.earliestDate = state.currentAccount.movements.sort(
      (a, b) => a.date - b.date
    )[0].date;
  } else {
...
export const updateTransaction = (newTransaction) => {
  ...
  Object.keys(transaction).forEach(
    (el) => (transaction[el] = newTransaction[el])
  );
  saveLocalStorage();
  return transaction;
};
