...
export const generatePieChartMarkup = (pie) => {
  let prevDegree = 0;
  const gradientString = pie
    .map((el) => {
      const add = `${colorsMap.get(`${el.name}`)} ${prevDegree}deg ${(
        prevDegree + el.degree
      ).toFixed(4)}deg`;
  ...
  return budgetArr
    .map((el) => {
      ...
      <div class="budget-item">
        <div class="budget-square" style="background-color: ${colorsMap.get(
          `${el.name}`
        )};"></div>
        <div>
